...
| `no-hardcoded-copy.spec.ts` names a sentence in your template                  | Add the key's English value through the Glossa MCP server (`set_translation`), reference it with `t()` — or `<app-prose>` if it contains code, emphasis or a link — then refresh the generated contract (`pnpm dev`/`pnpm build`). See AGENTS.md "Site copy and translations". |
