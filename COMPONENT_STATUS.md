...
Volt UI `1.1.0` is stable. Status labels describe the current confidence level for the copy-paste workflow, not a permanent promise — a `beta` label means more hardening may land in a minor release, not that the API can break outside a major.
...
- **beta**: usable, but may still need more form, keyboard, accessibility, or edge-case coverage before moving to stable. Reserved for components added after 1.0.
- **experimental**: reserved for future previews whose API or behavior may still change; not currently used.

## Release Groups

- **Stable**: every 1.0 component. The 1.0 surface ships entirely stable.
- **Beta**: `alert`, `spinner` (added in 1.1). Their APIs follow the 1.x promise; `beta` only means
  they have not yet had a release of real-world use.
- **Experimental**: none.
...
| alert           | beta   | n/a                       | n/a (static content)                            | opt-in `role="status"` / `"alert"`                                                       | yes  | unit + e2e consumer |
...
| button          | stable | n/a                       | native button; `voltButton` on `<button>`/`<a>` | focus-visible styles                                                                     | yes  | unit + e2e consumer |
...
| dialog          | stable | n/a                       | Escape covered in e2e                           | trigger, controlled and service-opened dialogs: focus trap/return, outside click covered | yes  | unit + e2e          |
...
| spinner         | beta   | n/a                       | n/a                                             | decorative by default, status when named                                                 | yes  | unit + e2e consumer |
...
| toast           | stable | n/a                       | close control covered                           | `VoltToastService` + pause and keyboard dismissal covered                                | yes  | unit + e2e          |
